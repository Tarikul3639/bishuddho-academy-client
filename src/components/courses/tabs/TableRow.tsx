        ...
        >
            ...
            <td className="hidden px-4 py-3 sm:table-cell">
                <p className="text-[13px] font-bold text-[#0d1b3e]">
                    ৳{student.payment?.amount?.toLocaleString() ?? 0}
                </p>
            ...
            <td className="px-4 py-3">
                {student.enrollment.status === "pending" ? (
                    <div className="flex items-center gap-1.5">
                        <button
                            disabled={isVerifying || isRejecting || !student.payment}
                            onClick={() =>
                                student.payment && onVerify(student.payment.paymentId)
                            }
                            className="flex items-center gap-1 rounded-sm bg-[#dcfce7] px-2.5 py-1.5 text-[11px] font-bold text-[#16a34a] transition hover:bg-[#bbf7d0] disabled:cursor-not-allowed disabled:opacity-60"
                        >
                            {isVerifying ? (
                        ...
                        <button
                            disabled={isVerifying || isRejecting || !student.payment}
                            onClick={() => onRejectClick(student)}
                            className="flex items-center gap-1 rounded-sm bg-[#fee2e2] px-2.5 py-1.5 text-[11px] font-bold text-[#ef4444] transition hover:bg-[#fecaca] disabled:cursor-not-allowed disabled:opacity-60"
                        >
                            {isRejecting ? (
                                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                            ) : (
