// app/admin/courses/[courseId]/components/tabs/StudentsTab.tsx
"use client";
...
export default function StudentsTab({
    ...
    const filtered = students.filter((s) => {
        const matchSearch = !search || [s.name, s.email].some((f) => f.toLowerCase().includes(search.toLowerCase()));
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
                {[
                    { label: `${students.length} Total`, bg: "#f3f4f6", color: "#374151" },
                    ...
                    <span key={b.label} className="rounded-full px-3 py-1 text-[11px] font-bold"
            ...
            <div className="flex flex-wrap items-center gap-2">
                <div className="relative flex-1">
                    ...
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
                        placeholder="Search student..." className="w-full rounded-lg border border-[#e5e7eb] bg-white py-2 pl-8 pr-4 text-[13px] outline-none placeholder:text-[#9ca3af] focus:border-[#1a56db]" />
                </div>
